...
function convertToFigmaCompatibleFormat(
    ...
    if ("letterCase" in converted) {
        ...
        if (
            typeof letterCaseValue === "string" &&
            letterCaseValue.includes("letterCase")
        ) {
    ...
    if (
        "textDecoration" in converted &&
        typeof converted.textDecoration === "string"
    ) {
        ...
        if (textDec.includes("textDecoration")) {
            ...
            let resolvedValue =
                resolveTokenReference(textDec, allTokens) || textDec
            ...
            converted.textDecoration = "strikethrough"
        }
    }
...
function resolveTokenReference(
    ...
    if (
        current &&
        typeof current === "object" &&
        ("$value" in current || "value" in current)
    ) {
...
function mergeStyleWithModifier(
    baseValue: Record<string, unknown>,
    modifierValue: Record<string, unknown>,
    allTokens?: Record<string, unknown>
): Record<string, unknown> {
...
function minifyDictionaryFigmaTypography(
    ...
    if (obj.hasOwnProperty(usesDtcg ? "$value" : "value")) {
        ...
        if (token.$type === "fontFamily") {
            if (
                ...
                if (Array.isArray(originalValue)) {
                    ...
                    tokenResult[usesDtcg ? "$value" : "value"] =
                        originalValue[0]
                } else {
            ...
            const valueKey = usesDtcg ? "$value" : "value"
            let tokenValue =
                originalValue !== undefined && usesReferences(originalValue)
                    ? originalValue
                    : usesDtcg
                      ? token.$value
                      : token.value
            ...
            if (allTokens && token.path) {
                ...
                if (lastSegment !== "default" && path.length > 1) {
                    ...
                    for (const segment of defaultPath) {
                        if (
                            defaultTokenObj &&
                            typeof defaultTokenObj === "object" &&
                            segment in defaultTokenObj
                        ) {
                            defaultTokenObj = (
                                defaultTokenObj as Record<string, unknown>
                    ...
                    if (
                        ...
                        defaultTokenObj &&
                        typeof defaultTokenObj === "object" &&
                        defaultTokenObj.hasOwnProperty(
                            usesDtcg ? "$value" : "value"
                        )
                    ) {
                        const defaultToken =
                            defaultTokenObj as TransformedToken
                        const defaultOriginalValue = usesDtcg
                            ? defaultToken.original?.$value
                            : defaultToken.original?.value

                        const defaultValue =
                            defaultOriginalValue !== undefined &&
                            usesReferences(defaultOriginalValue)
            ...
            if (typeof tokenValue === "object" && tokenValue !== null) {
                ...
                // Convert to Figma compatible format with resolved references
                tokenValue = convertToFigmaCompatibleFormat(
                    filteredValue,
                    allTokens
                )
                ...
                if (
                    Object.keys(tokenValue as Record<string, unknown>)
                        .length === 0
                ) {
                    return null // Signal that this token should be excluded
                }
        ...
        for (const name in obj) {
            if (obj.hasOwnProperty(name)) {
                const processed = minifyDictionaryFigmaTypography(
...
function generateCombinedFontWeights(
...
function generateFigmaCombinations(
    ...
    for (const { key: sectionKey, tokens: sectionTokens } of sections) {
        ...
        for (const [name, token] of Object.entries(sectionTokens)) {
            if (typeof token === "object" && token !== null) {
                ...
                if (isModifier(token)) {
                    ...
                    baseStyles.push({
                        name,
                        tokens: token as Record<string, any>,
                    })
                }
            }
        }
        ...
        for (const { name: baseName, tokens: baseTokens } of baseStyles) {
            ...
            for (const [variantName, variantToken] of Object.entries(
                baseTokens
            )) {
                    ...
                    if (Object.keys(filteredModifier).length > 0) {
                        ...
                        if (
                            ...
                            if ("fontWeight" in combinedValue) {
                                const fontWeightRef =
                                    combinedValue.fontWeight as string
                                if (
                                    typeof fontWeightRef === "string" &&
                                    fontWeightRef.startsWith(
                                        "{typography.weight."
                                    )
                                ) {
                                    // Convert {typography.weight.regular} to {typography.weight.regularItalic}
                                    const weightName = fontWeightRef
                            ...
                            combinedValue = convertToFigmaCompatibleFormat(
                                combinedValue,
                                allTokens
                            )
                        } else {
                            // For non-italic modifiers, use regular merging
                            combinedValue = mergeStyleWithModifier(
                                baseValue,
                                filteredModifier,
                                allTokens
                            )
...
export const jsonFigmaTypographyFormat: Format = {
    ...
    format: ({
        ...
        if (
            processedTokens.typography &&
            typeof processedTokens.typography === "object"
        ) {
            processedTokens = {
                ...processedTokens,
        ...
        const tokensWithCombinations = generateFigmaCombinations(
            processedTokens,
            usesDtcg,
            processedTokens
        )
        ...
        return (
            JSON.stringify(
                minifyDictionaryFigmaTypography(
                    tokensWithCombinations,
                    usesDtcg,
                    tokensWithCombinations
                ),
                null,
