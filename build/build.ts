...
import { getPlatform, TOKEN_PATHS_CONFIG } from "./config"
...
import { generateCSSIndex } from "./utils/css-index.util"
...
function getTokenPaths(tierName: string): ResolvedTokenPaths {
    const tier = TOKEN_PATHS_CONFIG[tierName]
    if (!tier) {
...
function getModeTokenPaths(
    ...
    if (!categoryModes) {
        throw new Error(
            `Category "${categoryName}" modes not found for tier "${tierName}"`
        )
    }
    ...
    if (!modeFilePath) {
        throw new Error(
            `Mode "${modeName}" not found for category "${categoryName}" in tier "${tierName}"`
        )
    }
    ...
    const primitiveSource = tier.source.filter((path) =>
        path.includes("/primitive/")
    )
...
function createModeConfig(
    ...
    return {
        ...
        platforms: createPlatformConfig(
            platform,
            tier,
            categoryName,
            true,
            `${tier}-${categoryName}-${modeName}`
        ),
    }
}
 ...
 */
async function buildModeTokens(
    tier: string,
    categoryName: string,
    modeName: string,
    platform: Platform = "css"
 ...
 * @throws Error if platform is not configured
 */
async function buildPrimitiveTokens(
    tier: string,
    categoryName: string,
    platform: Platform = "css"
): Promise<void> {
    ...
    const tierConfig = TOKEN_PATHS_CONFIG[tier]
    if (!tierConfig) {
    ...
    const primitiveSource = tierConfig.source.filter((path) =>
        path.includes("/primitive/")
    )
 ...
 * @throws Error if platform is not configured
 */
async function buildCategoryTokens(
    tier: string,
    category: string,
    platform: Platform = "css"
): Promise<void> {
...
function printUsage(): void {
    const availableTiers = Object.keys(TOKEN_PATHS_CONFIG)
    ...
    try {
        ...
        for (const [categoryName, categoryModes] of Object.entries(modes)) {
            for (const modeName of Object.keys(categoryModes)) {
                buildPromises.push(
                    buildModeTokens(tier, categoryName, modeName)
                )
            }
        }
        ...
        // Generate CSS index file with media queries
        console.log(`\n📄 Generating CSS index file for ${tier}...`)
        await generateCSSIndex(tier)
